type TBucketEl = [string, string | number]
type TStatus = {
  status: string,
  message: string
}
  ...
  set(key: string, value: string | number) {
    ...
    if (!item) {
      ...
      if (index >= 0) {
        ...
        this.log({
          status: 'success', message: `Данные по ключу ${key} перезаписаны`
        });
      } else {
        item.data = [...item.data, bucketEl];
        this.log({
          status: 'success', message: `В элемент баккета добавлено еще одно значение`
        });
      }
    }
  }
  ...
  delete(key: string) {
    ...
    if (bucket) {
      ...
      if (index >= 0) {
        ...
        this.log({status: 'success', message: `Значение по ключу ${key} удалено`});
      } else {
        this.log({
          status: 'error', message: `Значение по ключу ${key} не существует`
        });
      }
    }
  }
...
// const qqq: newBucket = new newBucket();
// qqq.set('name', 'Viktor');
